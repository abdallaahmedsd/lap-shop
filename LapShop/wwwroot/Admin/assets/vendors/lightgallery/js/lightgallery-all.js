...
(function() {
    ...
    function Plugin(element, options) {
        ...
        // Gallery elements
        if (this.s.dynamic) {
            this.$elements = this.s.dynamicEl;
        } else {
            if (this.s.selector === 'this') {
                this.$elements = this.$el;
            } else if (this.s.selector !== '') {
                if (this.s.selectWithin) {
                    this.$elements = $(this.s.selectWithin).find(this.s.selector);
                } else {
                    this.$elements = this.$el.find($(this.s.selector));
                }
            } else {
    ...
    Plugin.prototype.init = function() {
        ...
        // s.preload should not be more than $element.length
        if (_this.s.preload > _this.$elements.length) {
            _this.s.preload = _this.$elements.length;
        }
        ...
        if (_this.s.dynamic) {
            ...
            _this.$elements.on('click.lgcustom', function(event) {
                ...
                _this.index = _this.s.index || _this.$elements.index(this);
    ...
    Plugin.prototype.build = function(index) {
        ...
        if (_this.s.hideBarsDelay > 0) {
            ...
            _this.$outer.on('mousemove.lg click.lg touchstart.lg', function () {
                _this.$outer.removeClass('lg-hide-elements');

                clearTimeout(_this.hideBarTimeout);

                // Timeout will be cleared on each slide movement also
                _this.hideBarTimeout = setTimeout(function () {
                    _this.$outer.addClass('lg-hide-elements');
    ...
    Plugin.prototype.structure = function() {
        ...
        for (i = 0; i < this.$elements.length; i++) {
            list += '<div class="lg-element"></div>';
        }

        // Create controlls
        if (this.s.controls && this.$elements.length > 1) {
        ...
        this.$outer = $('.lg-outer');
        this.$outer.focus();
        this.$slide = this.$outer.find('.lg-element');
        ...
        if (this.s.enableDrag && this.$elements.length > 1) {
    ...
    Plugin.prototype.isVideo = function(src, index) {
        ...
        if (this.s.dynamic) {
            ...
            html = this.$elements.eq(index).attr('data-html');
        }
        ...
        if (!src) {
            if (html) {
                ...
                console.error('lightGallery :- data-src is not provided on slide element ' + (index + 1) + '. Please make sure the selector property is properly configured. More info - http://sachinchoolur.github.io/lightGallery/demos/html-markup.html');
                return false;
            }
        }
    ...
    Plugin.prototype.counter = function() {
        if (this.s.counter) {
            $(this.s.appendCounterTo).append('<div id="lg-counter" role="status" aria-live="polite"><span id="lg-counter-current">' + (parseInt(this.index, 10) + 1) + '</span> / <span id="lg-counter-all">' + this.$elements.length + '</span></div>');
        }
    };
    ...
    Plugin.prototype.addHtml = function(index) {
        ...
        if (this.s.dynamic) {
            ...
            $currentEle = this.$elements.eq(index);
    ...
    Plugin.prototype.preload = function(index) {
        ...
        for (i = 1; i <= this.s.preload; i++) {
            if (i >= this.$elements.length - index) {
                break;
            }
    ...
    Plugin.prototype.loadContent = function(index, rec, delay) {
        ...
        if (_this.s.dynamic) {
            ...
            var $currentEle = _this.$elements.eq(index);
        ...
        if (_this.s.dynamic) {
            ...
            if (_this.$elements.eq(index).attr('data-iframe') === 'true') {
                iframe = true;
            }
        }
    ...
    */
        ...
        if (!_this.lgBusy) {
            ...
            if (this.s.download) {
                ...
                if (_this.s.dynamic) {
                    ...
                    _src = _this.$elements.eq(index).attr('data-download-url') !== 'false' && (_this.$elements.eq(index).attr('data-download-url') || _this.$elements.eq(index).attr('href') || _this.$elements.eq(index).attr('data-src'));
    ...
    Plugin.prototype.goToPrevSlide = function(fromTouch) {
        ...
        if (!_this.lgBusy) {
            if (_this.index > 0) {
                ...
                if (_loop) {
                    _this.index = _this.$elements.length - 1;
                    _this.$el.trigger('onBeforePrevSlide.lg', [_this.index, fromTouch]);
                    _this.slide(_this.index, fromTouch, false, 'prev');
                } else if (_this.s.slideEndAnimatoin && !fromTouch) {
    ...
    Plugin.prototype.closeGallery = function() {
        ...
        if (_this.s.closable) {
            ...
            // for preventing this check mousedown and mouseup happened on .lg-element or lg-outer
            _this.$outer.on('mousedown.lg', function(e) {

                if ($(e.target).is('.lg-outer') || $(e.target).is('.lg-element ') || $(e.target).is('.lg-img-wrap')) {
                    mousedown = true;
                } else {
                    mousedown = false;
                }
            ...
            _this.$outer.on('mouseup.lg', function(e) {
                ...
                if ($(e.target).is('.lg-outer') || $(e.target).is('.lg-element ') || $(e.target).is('.lg-img-wrap') && mousedown) {
    ...
    Plugin.prototype.destroy = function(d) {
        ...
        if (d) {
            if (!_this.s.dynamic) {
                // only when not using dynamic mode is $elements a jquery collection
                this.$elements.off('click.lg click.lgcustom');
            }
...
(function() {
    ...
    var Autoplay = function(element) {
        ...
        // Execute only if elements are above 1
        if (this.core.$elements.length < 2) {
            return false;
        }
    ...
    Autoplay.prototype.startlAuto = function() {
        ...
        _this.interval = setInterval(function() {
            if (_this.core.index + 1 < _this.core.$elements.length) {
                _this.core.index++;
            } else {
                _this.core.index = 0;
            }
...
(function() {
    ...
    var Pager = function(element) {
        ...
        this.$el = $(element);
        this.core.s = $.extend({}, defaults, this.core.s);
        if (this.core.s.pager && this.core.$elements.length > 1) {
            this.init();
        }
    ...
    Pager.prototype.init = function() {
        ...
        if (_this.core.s.dynamic) {
            ...
            _this.core.$elements.each(function() {
...
(function() {
    ...
    var Thumbnail = function(element) {
        ...
        this.thumbTotalWidth = (this.core.$elements.length * (this.core.s.thumbWidth + this.core.s.thumbMargin));
    ...
    Thumbnail.prototype.init = function() {
        var _this = this;
        if (this.core.s.thumbnail && this.core.$elements.length > 1) {
    ...
    Thumbnail.prototype.build = function() {
        ...
        function getThumb(src, thumb, index) {
            ...
            thumbList += '<div data-vimeo-id="' + vimeoId + '" class="lg-thumb-element" style="width:' + _this.core.s.thumbWidth + 'px; height: ' + _this.core.s.thumbHeight + '; margin-right: ' + _this.core.s.thumbMargin + 'px"><img src="' + thumbImg + '" /></div>';
            vimeoId = '';
        }
        ...
        if (_this.core.s.dynamic) {
            ...
            _this.core.$elements.each(function(i) {
        ...
        $thumb = _this.core.$outer.find('.lg-thumb-element');
...
(function() {
        ...
        Video.prototype.init = function() {
            ...
            if (_this.core.doCss() && (_this.core.$elements.length > 1) && (_this.core.s.enableSwipe || _this.core.s.enableDrag)) {
        ...
        Video.prototype.loadVideo = function(src, addClass, noPoster, index, html) {
            ...
            if (_this.core.s.dynamic) {
                ...
                videoTitle = _this.core.$elements.eq(_this.core.index).attr('title') || _this.core.$elements.eq(_this.core.index).find('img').first().attr('alt');
            }
            ...
            if ($el.find('.lg-object').hasClass('lg-has-poster') && $el.find('.lg-object').is(':visible')) {
                ...
                if (!$el.hasClass('lg-has-video')) {
                    ...
                    if (_this.core.s.dynamic) {
                        ...
                        _src = _this.core.$elements.eq(_this.core.index).attr('href') || _this.core.$elements.eq(_this.core.index).attr('data-src');
                        _html = _this.core.$elements.eq(_this.core.index).attr('data-html');
        ...
        function onBeforeSlide(event, prevIndex, index) {
            ...
            if (_this.core.s.dynamic) {
                ...
                _src = _this.core.$elements.eq(index).attr('href') || _this.core.$elements.eq(index).attr('data-src');
    ...
    Zoom.prototype.init = function() {
        ...
        var actualSize = function(event, $image, index, fromIcon) {
            ...
            if (_this.core.s.dynamic) {
                ...
                nw = _this.core.$elements.eq(index).attr('data-width') || $image[0].naturalWidth || w;
            }
...
(function() {
    ...
    Share.prototype.getSahreProps = function(index, prop){
        ...
        if(this.core.s.dynamic) {
            ...
            var _href = this.core.$elements.eq(index).attr('href');
            var _prop = this.core.$elements.eq(index).data(prop);
...
(function () {
    ...
    Rotate.prototype.init = function () {
        ...
        // Save rotate config for each element to persist its rotate, flip values
