...
/*jshint -W083 */
	...
	$.jstree.defaults.core = {
		 ...
		 * data configuration
		 *
		 * If left as `false` the HTML inside the jstree container element is used to populate the tree (that should be an unordered list with list elements).
		 *
		...
		restore_focus : true,
		/**
		 * Force to compute and set "aria-setsize" and "aria-posinset" explicitly for each treeelement. 
	...
	$.jstree.core.prototype = {
		...
		init : function (el, options) {
			...
			this._data.core.original_container_html
				...
				.remove();
			this.element.html("<"+"ul class='jstree-container-ul jstree-children' role='group'><"+"li id='j"+this._id+"_loading' class='jstree-initial-node jstree-loading jstree-leaf jstree-last' role='none'><i class='jstree-icon jstree-ocl'></i><"+"a class='jstree-anchor' role='treeelement' href='#'><i class='jstree-icon jstree-themeicon-hidden'></i>" + this.get_string("Loading ...") + "</a></li></ul>");
		...
		_create_prototype_node : function () {
			...
			_temp1.setAttribute('role', 'treeelement');
      ...
      if (obj && $.inArray(obj.id, this._model.changed) === -1) {
		...
		deselect_node : function (obj, supress_event, e) {
			...
			if(obj.state.selected) {
				...
				this._data.core.selected = $.vakata.array_remove_element(this._data.core.selected, obj.id);
				if(dom.length) {
		...
		refresh : function (skip_loading, forget_state) {
			...
			if(!skip_loading) {
				this.element.html("<"+"ul class='"+c+"' role='group'><"+"li class='jstree-initial-node jstree-loading jstree-leaf jstree-last' role='none' id='j"+this._id+"_loading'><i class='jstree-icon jstree-ocl'></i><"+"a class='jstree-anchor' role='treeelement' href='#'><i class='jstree-icon jstree-themeicon-hidden'></i>" + this.get_string("Loading ...") + "</a></li></ul>");
				this.element.attr('aria-activedescendant','j'+this._id+'_loading');
			}
		...
		move_node : function (obj, par, pos, callback, is_loaded, skip_redraw, origin) {
			...
			if(obj.parent === new_par.id) {
				...
				old_ins._model.data[old_par].children = $.vakata.array_remove_element(old_ins._model.data[old_par].children, obj.id);
	...
	// remove element from array
	$.vakata.array_remove_element = function(array, element) {
		var tmp = $.inArray(element, array);
	...
	$.jstree.plugins.checkbox = function (options, parent) {
							...
							while(p && p.id !== $.jstree.root) {
								...
								if(c === j) {
									...
									if(p.state[ t ? 'selected' : 'checked' ]) {
										...
										this._data[ t ? 'core' : 'checkbox' ].selected = $.vakata.array_remove_element(this._data[ t ? 'core' : 'checkbox' ].selected, p.id);
										tmp = this.get_node(p, true);
										if(tmp && tmp.length) {
											tmp.attr('aria-selected', false).children('.jstree-anchor').removeClass(t ? 'jstree-clicked' : 'jstree-checked');
										}
		...
		this.uncheck_node = function (obj, e) {
			...
			if(obj.state.checked) {
				...
				this._data.checkbox.selected = $.vakata.array_remove_element(this._data.checkbox.selected, obj.id);
				if(dom.length) {
				...
				if(tmp && tmp.state.loaded) {
					for(k = 0, l = tmp.children_d.length; k < l; k++) {
						if(this._model.data[tmp.children_d[k]].state.checked) {
							c = true;
							this._data.checkbox.selected = $.vakata.array_remove_element(this._data.checkbox.selected, tmp.children_d[k]);
						}
					}
				}
			}
 ...
 */
	...
	$.jstree.defaults.contextmenu = {
		...
		show_at_node : true,
		/**
		 * an object of actions, or a function that accepts a node and a callback function and calls the callback function with an object of actions available for that node (you can also return the elements too).
		 *
		 * Each action consists of a key (a unique name) and a value which is an object with the following properties (only label and action are required). Once a menu element is activated the `action` function will be invoked with an object containing the following keys: element - the contextmenu element definition as seen below, reference - the DOM node that was used (the tree node), element - the contextmenu DOM element, position - an object with x/y properties indicating the position of the menu.
		 *
		 * * `separator_before` - a boolean indicating if there should be a separator before this element
		 * * `separator_after` - a boolean indicating if there should be a separator after this element
		 * * `_disabled` - a boolean indicating if this action should be disabled
		 * * `label` - a string - the name of the action (could be a function returning a string)
		 * * `title` - a string - an optional tooltip for the element
		 * * `action` - a function to be executed if this element is chosen, the function will receive 
		 * * `icon` - a string, can be a path to an icon or a className, if using an image that is in the current directory use a `./` prefix, otherwise it will be detected as a class
		 * * `shortcut` - keyCode which will trigger the action if the menu is open (for example `113` for rename, which equals F2)
		 * * `shortcut_label` - shortcut label (like for example `F2` for rename)
		 * * `submenu` - an object with the same structure as $.jstree.defaults.contextmenu.elements which can be used to create a submenu - each key will be rendered as a separate option in a submenu that will appear once the current element is hovered
		 *
		 * @name $.jstree.defaults.contextmenu.elements
		 * @plugin contextmenu
		 */
		elements : function (o, cb) { // Could be an object directly
			return {
				"create" : {
	...
	$.jstree.plugins.contextmenu = function (options, parent) {
		 ...
		 * @param {Number} x the x-coordinate relative to the document to show the menu at
		 * @param {Number} y the y-coordinate relative to the document to show the menu at
		 * @param {Number} i the object of elements to show
		 * @plugin contextmenu
		 * @trigger show_contextmenu.jstree
		 * @private
		 */
	...
	(function ($) {
		...
		$.vakata.context = {
			...
			_execute : function (i) {
				i = vakata_context.elements[i];
				return i && (!i._disabled || ($.vakata.is_function(i._disabled) && !i._disabled({ "element" : i, "reference" : vakata_context.reference, "element" : vakata_context.element }))) && i.action ? i.action.call(null, {
							"element"		: i,
							"reference"	: vakata_context.reference,
							"element"	: vakata_context.element,
							"position"	: {
			...
			_parse : function (o, is_callback) {
				...
				$.each(o, function (i, val) {
					if(!val) { return true; }
					vakata_context.elements.push(val);
					if(!sep && val.separator_before) {
						str += "<"+"li class='vakata-context-separator'><"+"a href='#' " + ($.vakata.context.settings.icons ? '' : 'class="vakata-context-no-icons"') + ">&#160;<"+"/a><"+"/li>";
					}
					sep = false;
					str += "<"+"li class='" + (val._class || "") + (val._disabled === true || ($.vakata.is_function(val._disabled) && val._disabled({ "element" : val, "reference" : vakata_context.reference, "element" : vakata_context.element })) ? " vakata-contextmenu-disabled " : "") + "' "+(val.shortcut?" data-shortcut='"+val.shortcut+"' ":'')+">";
					str += "<"+"a href='#' rel='" + (vakata_context.elements.length - 1) + "' " + (val.title ? "title='" + val.title + "'" : "") + ">";
					...
					str += ($.vakata.is_function(val.label) ? val.label({ "element" : i, "reference" : vakata_context.reference, "element" : vakata_context.element }) : val.label) + (val.shortcut?' <span class="vakata-contextmenu-shortcut vakata-contextmenu-shortcut-'+val.shortcut+'">'+ (val.shortcut_label || '') +'</span>':'') + "<"+"/a>";
					if(val.submenu) {
			...
			show : function (reference, position, data) {
				...
				if(vakata_context.elements.length) {
 ...
 */
	...
	$.jstree.defaults.dnd = {
		...
		touch : true,
		/**
		 * controls whether elements can be dropped anywhere on the node, not just on the anchor, by default only the node anchor is a valid drop target. Works best with the wholerow plugin. If enabled on mobile depending on the interface it might be hard for the user to cancel the drop, since the whole tree container will be a valid drop target.
			...
			this.element
				.on("search.jstree", function (e, data) {
						if(this._data.search.som && data.res.length) {
							...
							p = $.vakata.array_remove_element($.vakata.array_unique(p), $.jstree.root);
