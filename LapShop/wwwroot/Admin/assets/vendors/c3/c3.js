...
(function (global, factory) {
  ...
  var getPathBox = function (path) {
      var box = getBBox(path), elements = [path.pathSegList.getItem(0), path.pathSegList.getItem(1)], minX = elements[0].x, minY = Math.min(elements[0].y, elements[1].y);
  ...
  var CLASS = {
      ...
      legendItem: 'c3-legend-element',
      legendItemEvent: 'c3-legend-element-event',
      legendItemTile: 'c3-legend-element-tile',
      legendItemHidden: 'c3-legend-element-hidden',
      legendItemFocused: 'c3-legend-element-focused',
  ...
  (function () {
      ...
      if (!('SVGPathSegList' in window)) {
          ...
          window.SVGPathSegList.prototype.insertItemBefore = function (newItem, index) {
              this._checkPathSynchronizedToList();
              // Spec: If the index is greater than or equal to numberOfItems, then the new element is appended to the end of the list.
      ...
      if ($$.transiting) {
          interval = window.setInterval(function () {
              if (!$$.transiting) {
                  ...
                  if ($$.legend.selectAll('.c3-legend-element-focused').size() > 0) {
                      $$.expandArc(targetIds);
                  }
              }
          }, 10);
      ...
      if (hasGaugeType) {
          ...
          if ($$.filterTargetsToShow($$.data.targets).length === 1) {
              ...
              mainArcLabelLine
                  .style('fill', function (d) {
                  return $$.levelColor
                      ? $$.levelColor(d.data.values.reduce(function (total, element) {
  ...
  ChartInternal.prototype.getDefaultConfig = function () {
      var config = {
          ...
          legend_element_onclick: undefined,
          legend_element_onmouseover: undefined,
          legend_element_onmouseout: undefined,
          legend_equally: false,
          legend_padding: 0,
          legend_element_tile_width: 10,
          legend_element_tile_height: 10,
          // axis
          axis_rotated: false,
          axis_x_show: true,
          axis_x_type: 'indexed',
  ...
  ChartInternal.prototype.updateLegend = function (targetIds, options, transitions) {
      ...
      var paddingTop = 4, paddingRight = 10, maxWidth = 0, maxHeight = 0, posMin = 10, tileWidth = config.legend_element_tile_width + 5;
      ...
      function updatePositions(textElement, id, index) {
          var reset = index === 0, isLast = index === targetIds.length - 1, box = getTextBox(textElement, id), elementWidth = box.width +
              tileWidth +
              (isLast && !($$.isLegendRight || $$.isLegendInset) ? 0 : paddingRight) +
              config.legend_padding, elementHeight = box.height + paddingTop, elementLength = $$.isLegendRight || $$.isLegendInset ? elementHeight : elementWidth, areaLength = $$.isLegendRight || $$.isLegendInset
              ? $$.getLegendHeight()
              : $$.getLegendWidth(), margin, maxLength;
          // MEMO: care about condifion of step, totalLength
          function updateValues(id, withoutStep) {
              if (!withoutStep) {
                  margin = (areaLength - totalLength - elementLength) / 2;
                  if (margin < posMin) {
                      margin = (areaLength - elementLength) / 2;
                      totalLength = 0;
                      step++;
                  }
              }
          ...
          widths[id] = elementWidth;
          heights[id] = elementHeight;
          if (!maxWidth || elementWidth >= maxWidth) {
              maxWidth = elementWidth;
          }
          if (!maxHeight || elementHeight >= maxHeight) {
      ...
      xForLegendText = function (id, i) {
          return xForLegend(id, i) + 4 + config.legend_element_tile_width;
      };
      ...
      x2ForLegendTile = function (id, i) {
          return xForLegend(id, i) - 2 + config.legend_element_tile_width;
      };
      ...
      l = $$.legend
          ...
          .on('click', config.interaction_enabled
          ? function (id) {
              if (config.legend_element_onclick) {
                  config.legend_element_onclick.call($$, id);
              }
              else {
                  if ($$.d3.event.altKey) {
                      $$.api.hide();
          ...
          .on('mouseover', config.interaction_enabled
          ? function (id) {
              if (config.legend_element_onmouseover) {
                  config.legend_element_onmouseover.call($$, id);
              }
              else {
          ...
          .on('mouseout', config.interaction_enabled
          ? function (id) {
              if (config.legend_element_onmouseout) {
                  config.legend_element_onmouseout.call($$, id);
              }
              else {
      ...
      l.append('line')
          ...
          .attr('stroke-width', config.legend_element_tile_height);
      ...
      (withTransition ? tiles.transition() : tiles)
          ...
          ? function (id) {
              return $$.levelColor($$.cache[id].values.reduce(function (total, element) {
                  return total + element.value;
              }, 0));
          }
          : $$.color)
