...
(function () {
    ...
    var getValue = function (element) {
      return isString(element.value) ? element.value : '';
    };
    var getText = function (element) {
      if (isString(element.text)) {
        return element.text;
      } else if (isString(element.title)) {
    ...
    var sanitizeList = function (list, extractValue) {
      var out = [];
      global$4.each(list, function (element) {
        var text = getText(element);
        if (element.menu !== undefined) {
          var elements = sanitizeList(element.menu, extractValue);
          out.push({
            text: text,
            elements: elements
          });
    ...
    var isListGroup = function (element) {
      return hasNonNullableKey(element, 'elements');
    };
    var findTextByValue = function (value, catalog) {
      return findMap(catalog, function (element) {
        if (isListGroup(element)) {
          return findTextByValue(value, element.elements);
        } else {
          return someIf(element.value === value, element);
        }
      });
    };
    ...
    var getLinks = function (editor) {
      var extractor = function (element) {
        return editor.convertURL(element.value || element.url, 'href');
      };
      ...
      return new global$1(function (callback) {
        ...
        return optItems.bind(ListOptions.sanitizeWith(extractor)).map(function (elements) {
          if (elements.length > 0) {
            var noneItem = [{
                text: 'None',
                value: ''
              }];
            return noneItem.concat(elements);
          } else {
            return elements;
          }
        });
    ...
    var getRels = function (editor, initialTarget) {
      ...
      if (list.length > 0) {
        ...
        var safeRelExtractor = function (element) {
          return applyRelTargetRules(ListOptions.getValue(element), isTargetBlank_1);
        };
