...
(function () {
    ...
    var findItem = function (warehouse, element, comparator) {
      var filtered = filterItems(warehouse, function (detail) {
        return comparator(element, detail.element);
      });
    ...
    var eq = function (universe, element) {
      return curry(universe.eq, element);
    };
    ...
    var toGrid = function (warehouse, generators, isNew) {
      ...
      for (var rowIndex = 0; rowIndex < warehouse.grid.rows; rowIndex++) {
        ...
        for (var columnIndex = 0; columnIndex < warehouse.grid.columns; columnIndex++) {
          var element = Warehouse.getAt(warehouse, rowIndex, columnIndex).map(function (element) {
            return elementnew(element.element, isNew, element.isLocked);
          }).getOrThunk(function () {
    ...
    var modification = function (generators, toData) {
      ...
      var getOrInit = function (element, comparator) {
        return recent.fold(function () {
          ...
          return comparator(element, p.element) ? p.replacement : add(element);
        });
      };
      ...
      return function (generators) {
        ...
        var find = function (element, comparator) {
          return find$1(list, function (x) {
            return comparator(x.element, element);
          });
        };
        ...
        var replaceOrInit = function (element, comparator) {
          if (isRow$1(element) || isCol(element)) {
            ...
            return find(element, comparator).fold(function () {
              ...
              return comparator(element, p.element) ? p.sub : makeNew(element);
            });
          }
        };
    ...
    var isList$1 = function (universe, element) {
      var tagName = universe.property().name(element);
      return contains$2([
        'ol',
        'ul'
      ], tagName);
    };
    var isBlock$1 = function (universe, element) {
      var tagName = universe.property().name(element);
      return contains$2(blockList, tagName);
    };
    var isEmptyTag$1 = function (universe, element) {
      return contains$2([
        'br',
        'img',
        'hr',
        'input'
      ], universe.property().name(element));
    };
    ...
    var isListGroup = function (element) {
      return hasNonNullableKey(element, 'menu');
    };
    var buildListItems = function (elements) {
      return map$1(elements, function (element) {
        var text = element.text || element.title;
        if (isListGroup(element)) {
          return {
            text: text,
            elements: buildListItems(element.menu)
          };
        } else {
          return {
            text: text,
    ...
    var buildMenuItems = function (editor, selections, elements, format, onAction) {
      return map$1(elements, function (element) {
        var text = element.text || element.title;
        if (isListGroup(element)) {
          return {
            type: 'nestedmenuelement',
            text: text,
            getSubmenuItems: function () {
              return buildMenuItems(editor, selections, element.menu, format, onAction);
            }
          };
        } else {
          return {
            text: text,
            type: 'togglemenuelement',
            onAction: function () {
              return onAction(element.value);
            },
            onSetup: onSetupToggle(editor, selections, format, element.value)
          };
        }
      });
    };
    ...
    var filterNoneItem = function (list) {
      return bind$2(list, function (element) {
        if (isListGroup(element)) {
          return [__assign(__assign({}, element), { menu: filterNoneItem(element.menu) })];
        } else {
          return isNotEmpty(element.value) ? [element] : [];
        }
      });
    };
    var generateMenuItemsCallback = function (editor, selections, elements, format, onAction) {
      return function (callback) {
        return callback(buildMenuItems(editor, selections, elements, format, onAction));
      };
    };
    ...
    var getAdvancedTab = function (editor, dialogName) {
      ...
      var advTabItems = [
        {
          ...
          type: 'listbox',
          label: 'Border style',
          elements: emptyBorderStyle.concat(buildListItems(getTableBorderStyles(editor)))
        },
        {
          name: 'bordercolor',
          type: 'colorinput',
      ...
      var elements = dialogName === 'cell' ? [borderWidth].concat(advTabItems) : advTabItems;
      return {
        title: 'Advanced',
        name: 'advanced',
        elements: elements
    ...
    var getSharedValues = function (data) {
      ...
      each$2(comparisonData, function (elements) {
        each$2(keys(baseData), function (key) {
          each$1(elements, function (elementValue, elementKey) {
            var comparisonValue = baseData[key];
            if (comparisonValue !== '' && key === elementKey) {
              if (comparisonValue !== elementValue) {
                baseData[key] = '';
              }
            }
          });
    ...
    var applyStyleData$1 = function (editor, cells, data) {
      ...
      each$2(cells, function (element) {
        var cellElm = element.element;
        var modifier = isSingleCell ? DomModifier.normal(editor, cellElm) : DomModifier.ifTruthy(editor, cellElm);
        var colModifier = element.column.map(function (col) {
    ...
    var getData = function (editor, cells) {
      var cellsData = table(cells[0]).map(function (table) {
        return map$1(getSelectedCells(table, cells), function (element) {
          return extractDataFromCellElement(editor, element.element, hasAdvancedCellTab(editor), element.column);
        });
      });
    ...
    var open = function (editor, insertNewTable) {
      ...
      if (classes.length > 0) {
        if (data.class) {
          data.class = data.class.replace(/\s*mce\-element\-table\s*/g, '');
        }
      }
      var generalPanel = {
        type: 'grid',
        columns: 2,
        elements: getItems(editor, classes, insertNewTable)
      };
      var nonAdvancedForm = function () {
        return {
          type: 'panel',
          elements: [generalPanel]
        };
      };
    ...
    var traverse = function (element, mode) {
      return {
        element: element,
        mode: mode
      };
    };
    var backtrack = function (universe, element, _direction, transition) {
      if (transition === void 0) {
        transition = sidestep;
      }
      return universe.property().parent(element).map(function (p) {
        return traverse(p, transition);
      });
    };
    var sidestep = function (universe, element, direction, transition) {
      if (transition === void 0) {
        transition = advance;
      }
      return direction.sibling(universe, element).map(function (p) {
        return traverse(p, transition);
      });
    };
    var advance = function (universe, element, direction, transition) {
      if (transition === void 0) {
        transition = advance;
      }
      var children = universe.property().children(element);
    ...
    var go = function (universe, element, mode, direction, rules) {
      if (rules === void 0) {
        rules = successors;
      }
      ...
      return ruleOpt.bind(function (rule) {
        return rule.current(universe, element, direction, rule.next).orThunk(function () {
          return rule.fallback.bind(function (fb) {
            return go(universe, element, fb, direction);
          });
        });
      });
    };
    ...
    var left$1 = function () {
      var sibling = function (universe, element) {
        return universe.query().prevSibling(element);
      };
    ...
    var right$1 = function () {
      var sibling = function (universe, element) {
        return universe.query().nextSibling(element);
      };
      var first = function (children) {
    ...
    var hone = function (universe, element, predicate, mode, direction, isRoot) {
      var next = go(universe, element, mode, direction);
      return next.bind(function (n) {
        if (isRoot(n.element)) {
          return Optional.none();
        } else {
          return predicate(n.element) ? Optional.some(n.element) : hone(universe, n.element, predicate, n.mode, direction, isRoot);
        }
      });
    };
    var left = function (universe, element, predicate, isRoot) {
      return hone(universe, element, predicate, sidestep, Walkers.left(), isRoot);
    };
    var right = function (universe, element, predicate, isRoot) {
      return hone(universe, element, predicate, sidestep, Walkers.right(), isRoot);
    };
    ...
    var before$1 = function (universe, element, isRoot) {
      return seekLeft$1(universe, element, isLeaf(universe), isRoot);
    };
    var after$1 = function (universe, element, isRoot) {
      return seekRight$1(universe, element, isLeaf(universe), isRoot);
    };
    var seekLeft$1 = left;
