...
(function () {
    ...
    var filter = function (content, elements) {
      global$6.each(elements, function (v) {
        if (isRegExp(v)) {
      ...
      if (dataTransfer) {
        if (dataTransfer.getData) {
          ...
          if (legacyText && legacyText.length > 0) {
            if (legacyText.indexOf(mceInternalUrlPrefix) === -1) {
              elements['text/plain'] = legacyText;
            }
          }
        }
        if (dataTransfer.types) {
          for (var i = 0; i < dataTransfer.types.length; i++) {
            ...
            try {
              elements[contentType] = dataTransfer.getData(contentType);
            } catch (ex) {
    ...
    var isDataTransferItem = function (element) {
      return isNonNullable(element.getAsFile);
    };
    var readFilesAsDataUris = function (elements) {
      return global$8.all(map(elements, function (element) {
        return new global$8(function (resolve) {
          var blob = isDataTransferItem(element) ? element.getAsFile() : element;
    ...
    var getImagesFromDataTransfer = function (editor, dataTransfer) {
      var elements = dataTransfer.elements ? map(from(dataTransfer.elements), function (element) {
        return element.getAsFile();
      }) : [];
      var files = dataTransfer.files ? from(dataTransfer.files) : [];
      return filter$1(elements.length > 0 ? elements : files, isImage(editor));
    };
    ...
    var isBrokenAndroidClipboardEvent = function (e) {
      ...
      return navigator.userAgent.indexOf('Android') !== -1 && clipboardData && clipboardData.elements && clipboardData.elements.length === 0;
    };
