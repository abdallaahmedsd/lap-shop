...
(function () {
    ...
    var sanitizeList = function (list, extractValue) {
      var out = [];
      global.each(list, function (element) {
    ...
    var isGroup = function (element) {
      return has(element, 'elements');
    };
    var findEntryDelegate = function (list, value) {
      return findMap(list, function (element) {
        if (isGroup(element)) {
          return findEntryDelegate(element.elements, value);
    ...
    var collect = function (editor) {
      var urlListSanitizer = ListUtils.sanitizer(function (element) {
        return editor.convertURL(element.value || element.url, 'src');
      });
      var futureImageList = new global$3(function (completer) {
        createImageList(editor, function (imageList) {
          completer(urlListSanitizer(imageList).map(function (elements) {
            return flatten([
              [{
                  text: 'None',
                  value: ''
    ...
    var makeItems = function (info) {
      ...
      var imageList = info.imageList.map(function (elements) {
        return {
          name: 'images',
          type: 'listbox',
          label: 'Image list',
      ...
      var classList = info.classList.map(function (elements) {
        return {
          name: 'classes',
          type: 'listbox',
          label: 'Class',
    ...
    var makeDialogBody = function (info) {
      if (info.hasAdvTab || info.hasUploadUrl || info.hasUploadHandler) {
        ...
        var panel = {
          type: 'panel',
          elements: MainTab.makeItems(info)
        };
        return panel;
      }
    };
