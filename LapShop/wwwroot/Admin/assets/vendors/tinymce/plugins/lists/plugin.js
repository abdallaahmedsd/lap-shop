...
(function () {
    ...
    var outdentDlItem = function (editor, element) {
      if (isDescriptionDetail(element)) {
        mutate(element, 'dt');
      } else if (isDescriptionTerm(element)) {
        parent(element).each(function (dl) {
          return splitList(editor, dl.dom, element.dom);
        });
      }
    };
    var indentDlItem = function (element) {
      if (isDescriptionTerm(element)) {
        mutate(element, 'dd');
      }
    };
    var dlIndentation = function (editor, indentation, dlItems) {
      if (indentation === 'Indent') {
        ...
        each$1(dlItems, function (element) {
          return outdentDlItem(editor, element);
        });
      }
    };
    ...
    var joinSegment = function (parent, child) {
      append$1(parent.element, child.list);
    };
    ...
    var createSegment = function (scope, listType) {
      var segment = {
        ...
        element: SugarElement.fromTag('li', scope)
      };
      append$1(segment.list, segment.element);
      return segment;
    };
    ...
    var populateSegments = function (segments, entry) {
      for (var i = 0; i < segments.length - 1; i++) {
        set(segments[i].element, 'list-style-type', 'none');
      }
      last(segments).each(function (segment) {
        setAll(segment.list, entry.listAttributes);
        setAll(segment.element, entry.elementAttributes);
        append(segment.element, entry.content);
      });
    };
    ...
    var createItem = function (scope, attr, content) {
      var element = SugarElement.fromTag('li', scope);
      setAll(element, attr);
      append(element, content);
      return element;
    };
    var appendItem = function (segment, element) {
      append$1(segment.list, element);
      segment.element = element;
    };
    var writeShallow = function (scope, cast, entry) {
      var newCast = cast.slice(0, entry.depth);
      last(newCast).each(function (segment) {
        var element = createItem(scope, entry.elementAttributes, entry.content);
        appendItem(segment, element);
    ...
    var parseItem = function (depth, elementSelection, selectionState, element) {
      return firstChild(element).filter(isList).fold(function () {
        elementSelection.each(function (selection) {
          if (eq(selection.start, element)) {
            selectionState.set(true);
          }
        });
        var currentItemEntry = createEntry(element, depth, selectionState.get());
        elementSelection.each(function (selection) {
          if (eq(selection.end, element)) {
            selectionState.set(false);
          }
        });
        var childListEntries = lastChild(element).filter(isList).map(function (list) {
          return parseList(depth, elementSelection, selectionState, list);
        }).getOr([]);
    ...
    var parseList = function (depth, elementSelection, selectionState, list) {
      return bind(children(list), function (element) {
        var parser = isList(element) ? parseList : parseItem;
        var newDepth = depth + 1;
        return parser(newDepth, elementSelection, selectionState, element);
      });
    };
    var parseLists = function (lists, elementSelection) {
      var selectionState = Cell(false);
      var initialDepth = 0;
      return map(lists, function (list) {
        return {
          sourceList: list,
          entries: parseList(initialDepth, elementSelection, selectionState, list)
        };
      });
    };
    ...
    var updateListAttrs = function (dom, el, detail) {
      ...
      global$2.each(dom.select('li', el), function (li) {
        setAttribs(li, detail['list-element-attributes']);
      });
    };
