...
(function () {
    ...
    var flatten = function (xs) {
      ...
      for (var i = 0, len = xs.length; i < len; ++i) {
        if (!isArray(xs[i])) {
          throw new Error('Arr.flatten element ' + i + ' was not an array, input: ' + xs);
        }
    ...
    var focusItem = prefixName('focus.element');
    var tap = alloy.tap;
    ...
    var elementFields = constant$1([
      required$1('element'),
      required$1('selectedItem')
    ]);
    constant$1(objOf(elementFields().concat(menuFields())));
    var elementSchema$1 = constant$1(objOf(elementFields()));

    var _initSize = requiredObjOf('initSize', [
      required$1('numColumns'),
      required$1('numRows')
    ]);
    var elementMarkers = function () {
      return requiredOf('markers', elementSchema$1());
    };
    var tieredMenuMarkers = function () {
      return requiredObjOf('markers', [required$1('backgroundMenu')].concat(menuFields()).concat(elementFields()));
    };
    ...
    var getByIndex = function (component, hConfig, hState, index) {
      var elements = descendants(component.element, '.' + hConfig.elementClass);
      return Optional.from(elements[index]).fold(function () {
        return Result.error(new Error('No element found with index ' + index));
      }, component.getSystem().getByDom);
    };
    var getFirst = function (component, hConfig, _hState) {
      return descendant(component.element, '.' + hConfig.elementClass).bind(function (e) {
        return component.getSystem().getByDom(e).toOptional();
      });
    };
    var getLast = function (component, hConfig, _hState) {
      var elements = descendants(component.element, '.' + hConfig.elementClass);
      var last = elements.length > 0 ? Optional.some(elements[elements.length - 1]) : Optional.none();
    ...
    var getDelta = function (component, hConfig, hState, delta) {
      var elements = descendants(component.element, '.' + hConfig.elementClass);
      var current = findIndex$1(elements, function (element) {
        return has(element, hConfig.highlightClass);
      });
      return current.bind(function (selected) {
        var dest = cycleBy(selected, delta, 0, elements.length - 1);
        return component.getSystem().getByDom(elements[dest]).toOptional();
      });
    };
    ...
    var getCandidates = function (component, hConfig, _hState) {
      var elements = descendants(component.element, '.' + hConfig.elementClass);
      return cat(map$2(elements, function (i) {
    ...
    var highlights = function () {
      var get = function (component) {
        return Highlighting.getHighlighted(component).map(function (element) {
          return element.element;
        });
      };
      var set = function (component, element) {
        var prevFocus = get(component);
        component.getSystem().getByDom(element).fold(noop, function (element) {
          Highlighting.highlight(component, element);
        });
        var newFocus = get(component);
        reportFocusShifting(component, prevFocus, newFocus);
      };
    ...
    var getToolbarIconButton = function (clazz, editor) {
      ...
      return optOxideIcon.fold(function () {
        return dom$1('<span class="${prefix}-toolbar-button ${prefix}-toolbar-group-element ${prefix}-icon-' + clazz + ' ${prefix}-icon"></span>');
      }, function (icon) {
        return dom$1('<span class="${prefix}-toolbar-button ${prefix}-toolbar-group-element">' + icon + '</span>');
      });
    };
    ...
    var dataset = function () {
      ...
      var lookup = function (elementString) {
        return get$c(dataByValue.get(), elementString).orThunk(function () {
          return get$c(dataByText.get(), elementString);
        });
      };
      var update = function (elements) {
        ...
        each$1(elements, function (element) {
          newDataByValue[element.value] = element;
          get$c(element, 'meta').each(function (meta) {
            get$c(meta, 'text').each(function (text) {
              newDataByText[text] = element;
            });
          });
        });
    ...
    var sketch$3 = function (rawSpec) {
      ...
      var focusInput = function (dialog) {
        ...
        var dotelements = memDots.get(dialog);
        Highlighting.highlightAt(dotelements, spec$1.state.currentScreen.get());
      };
          ...
          formBehaviours: derive$2([
            ...
            config(formAdhocEvents, [
              runOnAttached(function (dialog, _simulatedEvent) {
                resetState();
                var dotelements = memDots.get(dialog);
                Highlighting.highlightFirst(dotelements);
      ...
      var memDots = record({
        ...
        behaviours: derive$2([Highlighting.config({
            ...
            elementClass: resolve('dot-element')
          })]),
        components: bind$3(spec$1.fields, function (_f, i) {
          return i <= spec$1.maxFieldIndex ? [spec('<div class="${prefix}-dot-element ${prefix}-icon-full-dot ${prefix}-icon"></div>')] : [];
        })
      });
      return {
    ...
    var build$2 = function (spec) {
      ...
      var element = renderToDom(modDefinition);
    ...
    var hoverEvent = 'alloy.element-hover';
    var focusEvent = 'alloy.element-focus';
    var onHover = function (element) {
      if (search(element.element).isNone() || Focusing.isFocused(element)) {
        if (!Focusing.isFocused(element)) {
          Focusing.focus(element);
        }
        emitWith(element, hoverEvent, { element: element });
      }
    };
    var onFocus = function (element) {
      emitWith(element, focusEvent, { element: element });
    };
    ...
    var builder$2 = function (detail) {
      return {
        dom: detail.dom,
        domModification: __assign(__assign({}, detail.domModification), { attributes: __assign(__assign(__assign({ 'role': detail.toggling.isSome() ? 'menuelementcheckbox' : 'menuelement' }, detail.domModification.attributes), { 'aria-haspopup': detail.hasSubmenu }), detail.hasSubmenu ? { 'aria-expanded': false } : {}) }),
        behaviours: SketchBehaviours.augment(detail.elementBehaviours, [
          ...
          config('element-type-events', __spreadArray(__spreadArray([], pointerEvents()), [
    ...
    var schema$5 = [
      ...
      option('toggling'),
      SketchBehaviours.field('elementBehaviours', [
        Toggling,
        Focusing,
        Keying,
        Representing
    ...
    var owner = constant$1('element-widget');
    ...
    var elementSchema = choose$1('type', {
      widget: schema$3,
      element: schema$5,
      separator: schema$4
    });
    var configureGrid = function (detail, movementInfo) {
      return {
        mode: 'flatgrid',
        selector: '.' + detail.markers.element,
        initSize: {
    ...
    var parts$2 = constant$1([group({
        factory: {
          sketch: function (spec) {
            var elementInfo = asRawOrDie$1('menu.spec element', elementSchema, spec);
            return elementInfo.builder(elementInfo);
          }
        },
        name: 'elements',
        unit: 'element',
        defaults: function (detail, u) {
          return has$2(u, 'uid') ? u : __assign(__assign({}, u), { uid: generate$2('element') });
        },
        overrides: function (detail, u) {
          return {
            type: u.type,
            ignoreFocus: detail.fakeFocus,
            domModification: { classes: [detail.markers.element] }
          };
        }
      })]);
      ...
      return {
        ...
        behaviours: augment(detail.menuBehaviours, [
          Highlighting.config({
            ...
            elementClass: detail.markers.element,
        ...
        events: derive$3([
          run(focus$1(), function (menu, simulatedEvent) {
            var event = simulatedEvent.event;
            menu.getSystem().getByDom(event.target).each(function (element) {
              Highlighting.highlight(menu, element);
              simulatedEvent.stop();
              emitWith(menu, focus(), {
                menu: menu,
                element: element
          ...
          run(hover(), function (menu, simulatedEvent) {
            var element = simulatedEvent.event.element;
            Highlighting.highlight(menu, element);
          })
        ]),
        components: components,
    ...
    var trace = function (elements, byItem, byMenu, finish) {
      return get$c(byMenu, finish).bind(function (triggerItem) {
        return get$c(elements, triggerItem).bind(function (triggerMenu) {
          var rest = trace(elements, byItem, byMenu, triggerMenu);
    ...
    var generate = function (menus, expansions) {
      ...
      each(menus, function (menuItems, menu) {
        each$1(menuItems, function (element) {
      ...
      var menuPaths = map$1(byMenu, function (_triggerItem, submenu) {
        return [submenu].concat(trace(elements, byItem, byMenu, submenu));
      });
      return map$1(elements, function (menu) {
    ...
    var init$2 = function () {
      ...
      var getTriggeringPath = function (elementValue, getItemByValue) {
        var extraPath = filter$2(lookupItem(elementValue).toArray(), function (menuValue) {
          return getPreparedMenu(menuValue).isSome();
        });
        return get$c(paths.get(), elementValue).bind(function (path) {
      ...
      var expand = function (elementValue) {
        return get$c(expansions.get(), elementValue).map(function (menu) {
          var current = get$c(paths.get(), elementValue).getOr([]);
          return [menu].concat(current);
        });
      };
      var collapse = function (elementValue) {
        return get$c(paths.get(), elementValue).bind(function (path) {
          return path.length > 1 ? Optional.some(path.slice(1)) : Optional.none();
        });
      };
      var refresh = function (elementValue) {
        return get$c(paths.get(), elementValue);
      };
      ...
      var lookupItem = function (elementValue) {
        return get$c(expansions.get(), elementValue);
      };
    ...
    var make$1 = function (detail, _rawUiSpec) {
      ...
      var getItemValue = function (element) {
        return Representing.getValue(element).value;
      };
      var getItemByValue = function (_container, menus, elementValue) {
        return findMap(menus, function (menu) {
          ...
          return find$2(candidates, function (c) {
            return getItemValue(c) === elementValue;
          });
        });
      };
      var toDirectory = function (_container) {
        return map$1(detail.data.menus, function (data, _menuName) {
          return bind$3(data.elements, function (element) {
            return element.type === 'separator' ? [] : [element.data.value];
          });
        });
      };
      var setActiveMenu = function (container, menu) {
        ...
        Highlighting.getHighlighted(menu).orThunk(function () {
          ...
          dispatch(container, element.element, focusItem());
        });
      };
      ...
      var getSubmenuParents = function (container) {
        return submenuParentItems.get().getOrThunk(function () {
          var r = {};
          var elements = descendants(container.element, '.' + detail.markers.element);
          var parentItems = filter$2(elements, function (i) {
            return get$b(i, 'aria-haspopup') === 'true';
          });
          each$1(parentItems, function (i) {
            container.getSystem().getByDom(i).each(function (elementComp) {
              var key = getItemValue(elementComp);
              r[key] = elementComp;
            });
          });
      ...
      var expandRight = function (container, element, decision) {
        if (decision === void 0) {
          decision = ExpandHighlightDecision.HighlightSubmenu;
        }
        if (element.hasConfigured(Disabling) && Disabling.isDisabled(element)) {
          ...
          return layeredState.expand(value).bind(function (path) {
            ...
            return Optional.from(path[0]).bind(function (menuName) {
              return layeredState.lookupMenu(menuName).bind(function (activeMenuPrep) {
                ...
                detail.onOpenSubmenu(container, element, activeMenu, reverse(path));
      ...
      var collapseLeft = function (container, element) {
        var value = getItemValue(element);
        return layeredState.collapse(value).bind(function (path) {
          updateAriaExpansions(container, path);
          return updateMenuPath(container, layeredState, path).map(function (activeMenu) {
            detail.onCollapseMenu(container, element, activeMenu);
            return activeMenu;
          });
        });
      };
      var updateView = function (container, element) {
        var value = getItemValue(element);
      ...
      var onRight = function (container, element) {
        return inside(element.element) ? Optional.none() : expandRight(container, element, ExpandHighlightDecision.HighlightSubmenu);
      };
      var onLeft = function (container, element) {
        return inside(element.element) ? Optional.none() : collapseLeft(container, element);
      };
      var onEscape = function (container, element) {
        return collapseLeft(container, element).orThunk(function () {
          return detail.onEscape(container, element).map(function () {
            return container;
          });
        });
      };
      var keyOnItem = function (f) {
        return function (container, simulatedEvent) {
          return closest$1(simulatedEvent.getSource(), '.' + detail.markers.element).bind(function (target) {
            return container.getSystem().getByDom(target).toOptional().bind(function (element) {
              return f(container, element).map(always);
            });
          });
        };
      };
      var events = derive$3([
        run(focus(), function (sandbox, simulatedEvent) {
          var element = simulatedEvent.event.element;
          layeredState.lookupItem(getItemValue(element)).each(function () {
            var menu = simulatedEvent.event.menu;
            Highlighting.highlight(sandbox, menu);
            var value = getItemValue(simulatedEvent.event.element);
        ...
        runOnExecute(function (component, simulatedEvent) {
          ...
          component.getSystem().getByDom(target).each(function (element) {
            var elementValue = getItemValue(element);
            if (elementValue.indexOf('collapse-element') === 0) {
              collapseLeft(component, element);
            }
            expandRight(component, element, ExpandHighlightDecision.HighlightSubmenu).fold(function () {
              detail.onExecute(component, element);
            }, noop);
          });
        }),
        runOnAttached(function (container, _simulatedEvent) {
          ...
          var element = simulatedEvent.event.element;
          updateView(sandbox, element);
          expandRight(sandbox, element, ExpandHighlightDecision.HighlightParent);
          detail.onHover(sandbox, element);
        })] : []));
      ...
      return {
        ...
        behaviours: augment(detail.tmenuBehaviours, [
          ...
          Highlighting.config({
            ...
            elementClass: detail.markers.menu
          }),
          Composing.config({
    ...
    var collapseItem$1 = constant$1('collapse-element');
    ...
    var getValue = function (element) {
      return get$c(element, 'format').getOr(element.title);
    };
    var convert = function (formats, memMenuThunk) {
      var mainMenu = makeMenu('Styles', [].concat(map$2(formats.elements, function (k) {
      ...
      var submenus = map$1(formats.menus, function (menuItems, menuName) {
        var elements = map$2(menuItems, function (element) {
          return makeItem(getValue(element), element.title, element.isSelected !== undefined ? element.isSelected() : false, element.getPreview !== undefined ? element.getPreview() : '', hasNonNullableKey(formats.expansions, getValue(element)));
        });
        return makeMenu(menuName, elements, memMenuThunk, true);
      });
    ...
    var makeItem = function (value, text, selected, preview, isMenu) {
      return {
        ...
        dom: {
          tag: 'div',
          classes: isMenu ? [resolve('styles-element-is-menu')] : []
        },
        toggling: {
          toggleOnExecute: false,
        ...
        elementBehaviours: derive$2(isMenu ? [] : [format(value, function (comp, status) {
            var toggle = status ? Toggling.on : Toggling.off;
            toggle(comp);
          })]),
        components: [{
    ...
    var makeMenu = function (value, elements, memMenuThunk, collapsable) {
      return {
        value: value,
        dom: { tag: 'div' },
        components: [
          Button.sketch({
            ...
            action: function (element) {
              if (collapsable) {
                var comp = memMenuThunk().get(element);
          ...
          }),
          {
            dom: {
              tag: 'div',
              classes: [resolve('styles-menu-elements-container')]
            },
            components: [Menu.parts.elements({})],
    ...
    var sketch$1 = function (settings) {
      ...
      var memMenu = record(tieredMenu.sketch({
        ...
        fakeFocus: true,
        stayInDom: true,
        onExecute: function (_tmenu, element) {
          var v = Representing.getValue(element);
          settings.handle(element, v.value);
          return Optional.none();
        },
        onEscape: function () {
          return Optional.none();
        ...
        onOpenSubmenu: function (container, element, submenu) {
          var w = get$5(container.element);
          var menu = ancestor(element.element, '[role="menu"]').getOrDie('hacky');
        ...
        onCollapseMenu: function (container, element, menu) {
          var submenu = ancestor(element.element, '[role="menu"]').getOrDie('hacky');
        ...
        markers: {
          ...
          element: resolve('styles-element'),
          selectedItem: resolve('styles-selected-element')
        }
      }));
      return memMenu.asSpec();
    };

    var getFromExpandingItem = function (element) {
      var newItem = deepMerge(exclude(element, ['elements']), { menu: true });
      var rest = expand(element.elements);
      var newMenus = deepMerge(rest.menus, wrap(element.title, rest.elements));
      var newExpansions = deepMerge(rest.expansions, wrap(element.title, element.title));
      return {
        element: newItem,
        menus: newMenus,
    ...
    var getFromItem = function (element) {
      return hasNonNullableKey(element, 'elements') ? getFromExpandingItem(element) : {
        element: element,
        menus: {},
        expansions: {}
      };
    ...
    var expand = function (elements) {
      return foldr(elements, function (acc, element) {
        var newData = getFromItem(element);
        return {
          menus: deepMerge(acc.menus, newData.menus),
          elements: [newData.element].concat(acc.elements),
    ...
    var register$1 = function (editor) {
      ...
      var enrichSupported = function (element) {
        return deepMerge(element, {
          isSelected: isSelectedFor(element.format),
          getPreview: getPreview(element.format)
        });
      };
      var enrichMenu = function (element) {
        return deepMerge(element, {
          isSelected: never,
          getPreview: constant$1('')
        });
      };
      var enrichCustom = function (element) {
        var formatName = generate$4(element.title);
        var newItem = deepMerge(element, {
          format: formatName,
      ...
      var doEnrich = function (elements) {
        return map$2(elements, function (element) {
          if (hasNonNullableKey(element, 'elements')) {
            var newItems = doEnrich(element.elements);
            return deepMerge(enrichMenu(element), { elements: newItems });
          } else if (hasNonNullableKey(element, 'format')) {
            return enrichSupported(element);
          } else {
            return enrichCustom(element);
          }
        });
      };
    ...
    var prune = function (editor, formats) {
      var doPrune = function (elements) {
        return bind$3(elements, function (element) {
          if (element.elements !== undefined) {
            var newItems = doPrune(element.elements);
            return newItems.length > 0 ? [element] : [];
          } else {
            var keep = hasNonNullableKey(element, 'format') ? editor.formatter.canApply(element.format) : true;
            return keep ? [element] : [];
          }
        });
      };
    ...
    var identifyFromArray = function (toolbar) {
      return bind$3(toolbar, function (element) {
        return isArray(element) ? identifyFromArray(element) : extract(element);
      });
    };
    ...
    var detect = function (editor, features) {
      var elementNames = identify(editor);
      var present = {};
      return bind$3(elementNames, function (iName) {
    ...
    var factory = function (detail, components, _spec, _externals) {
      return {
        ...
        behaviours: augment(detail.tgroupBehaviours, [Keying.config({
            mode: 'flow',
            selector: detail.markers.elementSelector
          })]),
    ...
    var ScrollingToolbar = function () {
      var makeGroup = function (gSpec) {
        ...
        return {
          ...
          components: [Container.sketch({ components: [ToolbarGroup.parts.elements({})] })],
          markers: { elementSelector: '.' + resolve('toolbar-group-element') },
          elements: gSpec.elements
        };
      };
          ...
          3: [
            function (require, module, exports) {
              (function (setImmediate, clearImmediate) {
                ...
                exports.enroll = function (element, msecs) {
                  clearTimeout(element._idleTimeoutId);
                  element._idleTimeout = msecs;
                };
                exports.unenroll = function (element) {
                  clearTimeout(element._idleTimeoutId);
                  element._idleTimeout = -1;
                };
                exports._unrefActive = exports.active = function (element) {
                  clearTimeout(element._idleTimeoutId);
                  var msecs = element._idleTimeout;
                  if (msecs >= 0) {
                    element._idleTimeoutId = setTimeout(function onTimeout() {
                      if (element._onTimeout)
                        element._onTimeout();
                    }, msecs);
                  }
                };
      ...
      var renderUI = function () {
        ...
        editor.on('init', function () {
          ...
          var backToMaskGroup = {
            ...
            scrollable: false,
            elements: [forToolbar('back', function () {
                editor.selection.collapse();
                realm.exit();
              }, {}, editor)]
          };
          var backToReadOnlyGroup = {
            ...
            scrollable: false,
            elements: [forToolbar('readonly-back', function () {
          ...
          var elements = detect(editor, features);
          var actionGroup = {
