(function($) {
  ...
  if ($(".flot-chart-line").length) {
    $(".flot-chart-line").bind("plothover", function(event, pos, element) {
      if (element) {
        var x = element.datapoint[0].toFixed(2),
          y = element.datapoint[1].toFixed(2);
        $(".flot-tooltip").html(element.series.label + " Sales " + " : " + y).css({
          top: element.pageY + 5,
          left: element.pageX + 5
        }).show();
      } else {
        $(".flot-tooltip").hide();
      }
